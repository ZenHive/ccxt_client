defmodule CCXT.Types.Schema.Balances do
  @moduledoc """
  Auto-generated map type alias for `CCXT.Types.Balances`.

  Source: priv/ccxt/ts/src/base/types.ts

  This is a dictionary/map type (not a struct). It defines `@type t` only.

  Do not edit this file manually - it will be overwritten by `mix ccxt.gen.types`.

  """

  @fields [
    %{name: :raw, optional: false, source: "info", type: "any()"},
    %{name: :timestamp, optional: true, source: "timestamp", type: "any()"},
    %{name: :datetime, optional: true, source: "datetime", type: "any()"}
  ]
  @type_spec_str """
  %{
    required(:raw) => any(),
    optional(:timestamp) => any(),
    optional(:datetime) => any(),
    optional(String.t()) => CCXT.Types.BalanceEntry.t()
  }
  """
  @type_spec Code.string_to_quoted!(@type_spec_str)

  defmacro __using__(_opts) do
    fields = @fields
    type_spec = @type_spec

    quote do
      @fields unquote(Macro.escape(fields))
      @type_spec unquote(Macro.escape(type_spec))
      @type t :: unquote(type_spec)
    end
  end
end

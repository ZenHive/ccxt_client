defmodule CCXT.Types.Schema.DepositWithdrawFee do
  @moduledoc """
  Auto-generated schema for `CCXT.Types.DepositWithdrawFee`.

  Source: priv/ccxt/ts/src/base/types.ts

  ## Fields

  - `raw` - any()
  - `withdraw` - CCXT.Types.DepositWithdrawFeeNetwork.t() (optional)
  - `deposit` - CCXT.Types.DepositWithdrawFeeNetwork.t() (optional)
  - `networks` - %{optional(String.t()) => CCXT.Types.DepositWithdrawFeeNetwork.t()} (optional)

  ## Usage

  This module is used by `CCXT.Types.DepositWithdrawFee`:

      defmodule CCXT.Types.DepositWithdrawFee do
        use CCXT.Types.Schema.DepositWithdrawFee
      end

  Do not edit this file manually - it will be overwritten by `mix ccxt.gen.types`.

  """

  Module.register_attribute(__MODULE__, :fields, persist: true)

  @fields [
    %{name: :raw, optional: false, source: "info", type: "any()"},
    %{
      name: :withdraw,
      optional: true,
      source: "withdraw",
      type: "CCXT.Types.DepositWithdrawFeeNetwork.t()"
    },
    %{
      name: :deposit,
      optional: true,
      source: "deposit",
      type: "CCXT.Types.DepositWithdrawFeeNetwork.t()"
    },
    %{
      name: :networks,
      optional: true,
      source: "networks",
      type: "%{optional(String.t()) => CCXT.Types.DepositWithdrawFeeNetwork.t()}"
    }
  ]
  @type_spec_str """
  %__MODULE__{
    raw: any(),
    withdraw: CCXT.Types.DepositWithdrawFeeNetwork.t() | nil,
    deposit: CCXT.Types.DepositWithdrawFeeNetwork.t() | nil,
    networks: %{optional(String.t()) => CCXT.Types.DepositWithdrawFeeNetwork.t()} | nil
  }
  """
  @type_spec Code.string_to_quoted!(@type_spec_str)

  defmacro __using__(opts) do
    fields = @fields
    type_spec = @type_spec
    struct_defaults = Keyword.get(opts, :struct_defaults, [])
    field_names = Enum.map(fields, & &1.name)

    struct_fields =
      Enum.map(field_names, fn name ->
        if Keyword.has_key?(struct_defaults, name) do
          {name, Keyword.fetch!(struct_defaults, name)}
        else
          name
        end
      end)

    quote do
      import CCXT.Types.Helpers, only: [get_value: 2, get_camel_value: 3]

      @fields unquote(Macro.escape(fields))
      @type_spec unquote(Macro.escape(type_spec))
      @type t :: unquote(type_spec)
      defstruct unquote(struct_fields)
      @spec from_map(map()) :: t()
      def from_map(map) when is_map(map) do
        %__MODULE__{
          raw: get_camel_value(map, :raw, :info),
          withdraw: get_value(map, :withdraw),
          deposit: get_value(map, :deposit),
          networks: get_value(map, :networks)
        }
      end

      defoverridable from_map: 1
    end
  end
end
